...
class DataStream:
    ...
    def __init__(
        ...
        self.logger.info(
            f"Run ID: {self.data_store.run_id} initiated: {self.data_store.start_time}..."
        )
    ...
    def run(self) -> None:
        for step in self.config.steps:
            if "extract" in step.type:
                extracted_data = Extractor.extract(step, self.data_store.step_outputs)
                self.data_store[step.output] = extracted_data

            if "transform" in step.type:
                input_data = {
                    item: self.data_store.step_outputs.get(item) for item in step.input
                }
                output_data = step.function(input_data)
                self.data_store.step_outputs.update(output_data)

            if "load" in step.type:
                dest_response = Loader.load(step, self.data_store.step_outputs)
